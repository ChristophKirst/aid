...
from .relative_position_representation import TransformerEncoderRPR, TransformerEncoderLayerRPR

from aid.utils.midi_encoder import ENCODE_SIZE, ENCODE_TOKEN_END, ENCODE_TOKEN_PAD
...
class MusicTransformer(nn.Module):
    ...
    def __init__(self, n_layers=6, n_heads=8, d_model=512, d_feedforward=1024,
        ...
        # Input embedding
        self.embedding = nn.Embedding(ENCODE_SIZE, self.d_model)
        ...
        if(not self.rpr):
            ...
            self.transformer = nn.Transformer(
                d_model=self.d_model, nhead=self.n_heads, num_encoder_layers=self.n_layers,
        ...
        # RPR Transformer
            ...
            encoder_layer = TransformerEncoderLayerRPR(self.d_model, self.n_heads, self.d_feedforward, self.dropout, er_len=self.max_seq)
            encoder = TransformerEncoderRPR(encoder_layer, self.n_layers, encoder_norm)
            self.transformer = nn.Transformer(
                d_model=self.d_model, nhead=self.n_heads, num_encoder_layers=self.n_layers,
                num_decoder_layers=0, dropout=self.dropout, # activation=self.ff_activ,
                dim_feedforward=self.d_feedforward, custom_decoder=self.dummy, custom_encoder=encoder
            )

        # Final output is a softmaxed linear layer
        self.Wout       = nn.Linear(self.d_model, ENCODE_SIZE)
    ...
    def forward(self, x, mask=True):
        ...
        if(mask is True):
            mask = self.transformer.generate_square_subsequent_mask(x.shape[1]).to(device)
        else:
            mask = None
    ...
    def generate(self, primer=None, target_seq_length=1024, beam=0, beam_chance=1.0):
        ...
        gen_seq = torch.full((1,target_seq_length), ENCODE_TOKEN_PAD, dtype=torch.long, device=device)
        ...
        while(cur_i < target_seq_length):
            ...
            y = self.softmax(self.forward(gen_seq[..., :cur_i]))[..., :ENCODE_TOKEN_PAD]
            ...
            if(beam_ran <= beam_chance):
                ...
                beam_rows = top_i // ENCODE_SIZE
                beam_cols = top_i % ENCODE_SIZE
                ...
                if(next_token == ENCODE_TOKEN_END):
